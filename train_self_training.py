...
def run_test(cfg, feature_extractor, classifier, local_rank, distributed):
    ...
    with torch.no_grad():
        ...
        for i, (x, y, _) in enumerate(test_loader):
            ...
            f1_sum += f1(output, y, n_classes=6)
            acc_sum += OA(output, y, n_classes=6)
        ...
        logger.info('Test result: mIoU/mF1/oACC {:.5f}/{:.5f}/{:.5f}.'.format(mIoU, mF1, oACC))
        for i in range(cfg.MODEL.NUM_CLASSES):
            logger.info("Class_{} {} Result: class_iou/class_f1/class_acc {:.5f}/{:.5f}/{:.5f}.".format(i, test_data.trainid2name[i], class_iou[i], class_f1[i], class_acc[i]))
