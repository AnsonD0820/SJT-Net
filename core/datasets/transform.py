...
class Resize(object):
    ...
    def __call__(self, image, label):
        image = F.resize(image, self.size, InterpolationMode.BICUBIC)
...
class RandomScale(object):
    ...
    def __call__(self, image, label):
        ...
        image = F.resize(image, size, InterpolationMode.BICUBIC)
        if self.resize_label:
            if isinstance(label, np.ndarray):
                # assert the shape of label is in the order of (h, w, c)
                label = cv2.resize(label,(self.size[1], self.size[0]), cv2.INTER_LINEAR)
            else:
                label = F.resize(label, size, InterpolationMode.NEAREST)
        return image, label
